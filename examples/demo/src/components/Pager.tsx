...
export function Pager(_: unknown, { update }: Ctx) {
  let page = 1;
  ...
  return (props: Record<string, unknown> | undefined) => {
    ...
    return (
      <div
        class="pager"
        style="padding: 12px; border: 1px solid #6366f1; border-radius: 8px; display: inline-block;"
      >
        <p>
          Current page: <strong>{String(current)}</strong>
        </p>
        <p style="color: #888; font-size: 12px;">render(props) received: {JSON.stringify(props)}</p>
