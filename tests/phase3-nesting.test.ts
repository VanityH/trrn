 ...
 *
 * 验证点：
 * 1. trrn 父组件内嵌普通 HTML 元素
 * 2. trrn 父组件内嵌另一个 trrn 组件
 * 3. 嵌套组件的状态彼此独立
 * 4. 嵌套组件可以触发父组件更新
 */
...
test("父组件 render 返回包含 HTML 元素的 VNode", () => {
  ...
  const Parent = defineComponent((_props, _ctx) => {
    return (_p) =>
...
test("子组件通过回调触发父组件状态更新", async () => {
  ...
  const ChildBtn = defineComponent((p, _ctx) => {
    const { label } = p ?? { label: "?" };
    return (p) =>
      h(
        "button",
...
test("三层嵌套组件", () => {
  ...
  const Middle = defineComponent((_props, _ctx) => {
    return (p) =>
      h(
        "div",
        { class: "middle" },
  ...
  const Root = defineComponent((_props, _ctx) => {
